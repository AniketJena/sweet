<script lang="ts">
	...
	let nameErrorRing = $derived(nameErrors.length > 0);
	...
	let confirmPasswordErrors = $state('');
	let confirmPasswordErrorRing = $derived(confirmPasswordErrors.length > 0);
	...
	const handleSubmit = async (e: SubmitEvent) => {
		...
		const confirmPassword = f.get('confirm-password');

		if (password !== confirmPassword) {
			confirmPasswordErrors = "Password doesn't match";
			isLoading = false;
			return;
		}
		...
		try {
			...
			if (axios.isAxiosError(err)) {
				...
				if (err.response?.data.name) {
					nameErrors = err.response?.data.name[0];
				}
...
<form onsubmit={(e) => handleSubmit(e)} class="mx-auto mt-24 flex w-[32rem] flex-col gap-5">
	<div class="grid gap-1">
		<label for="name" class:text-rose-500={nameErrorRing} class="select-none text-sm">Name</label>
		<input
			id="name"
			class:err-ring={nameErrorRing}
	...
	<div class="relative grid gap-1">
		...
		<label
			...
			class:text-rose-500={confirmPasswordErrorRing}
			class="select-none text-sm">Password</label
		>
		<input
			id="confirm-password"
			class:err-ring={confirmPasswordErrorRing}
